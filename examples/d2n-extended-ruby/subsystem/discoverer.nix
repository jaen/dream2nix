{
    ...
    gemspecName =
      ...
      then let
        gemSpecArgName = let
          ...
          match =
            if gemSpecArgNameMatch != null
            then (l.head gemSpecArgNameMatch)
            else null;
        in
          match;
        gemName =
          if gemSpecArgName != null
          then let
            gemNameMatch = l.match ".+[[:space:]]*${escapeRegex gemSpecArgName}\.name[[:space:]]+=[[:space:]]+\"([^\"]+)\".+" gemspec.content;
            match =
              if gemNameMatch != null
              then (l.head gemNameMatch)
              else null;
          in
            match
          else null;
  ...
  #   [ project ];
in {
  discover = args: lib.traceValFn (x: "RUBY DISCOVERED: ${l.toJSON x}") (discover args);
}
